...
export default async function PreparePage({ params }: { params: Promise<{ noteId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 space-y-6">
      ...
      {briefContent ? (
        // Unit 48 PR3 — intent-aware dispatch (Decision 11 / sibling
        // pattern). When the brief was generated by the intent-aware
        // path (worker stamped `briefContent.intent` AND the pair is
        // in SUPPORTED_INTENT_PAIRS), render the IntentAwareBriefCard
        // wrapper which injects spine components via spineSlot.
        // Otherwise render the existing BriefCard untouched — which is
        // every pre-PR3 brief and every brief whose pair isn't yet
        // supported.
        briefContent.intent &&
        isIntentAwarePairSupported(note.division, briefContent.intent) ? (
          <IntentAwareBriefCard
            content={briefContent}
            patientName={`${note.patient.firstName} ${note.patient.lastName}`.trim()}
            intent={briefContent.intent}
            nowMs={nowMs}
          />
        ) : (
          <BriefCard
