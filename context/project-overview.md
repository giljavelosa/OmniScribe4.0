...
`/login` → `/home` (pick patient) → `/prepare/[noteId]` (read brief, confirm setup) → `/capture/[noteId]` (record, mid-visit start drafting) → `/processing/[noteId]` (transient) → `/review/[noteId]` (edit + regenerate sections + close follow-ups) → `/sign/[noteId]` (signing-PIN re-verify, sign) → `/home` (next patient).
...
- Sign-in (email + password), signing PIN for sign-time re-auth, password reset, admin-initiated password reset (audited).
...
- **WebAuthn / hardware-key second factor** — password + signing PIN only in v1.
...
1. **Onboarding** — a platform owner can provision a new Organization with BAA fields, invite a first ORG_ADMIN, and that admin can accept the invite, set a password, and set a signing PIN without leaving the product. (Journey 07 + 01)
